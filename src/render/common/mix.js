...
export function checkinput$x(
  ...
  const toData = () => {
    var result = Object.keys(data)
      .filter(i => !/^\$/.test(i))
      .map(i => ({ label: i, value: data[i], $value: data[`$${i}`] }));
    result.$data = data;
    return result;
  };
  ...
  const handleCheck = (e, { name, checked, option }) => {
    ...
    return onChange(e, toData()).then(() =>
      onBlur({ checkedChange: true }, `${name}-checkbox`)
    );
  };
  ...
  const handleInput = (e, { value, name, target }) => {
    data[`$${name}`] = value;
    data[name] = value;
    onChange(e, toData()).then(() =>
      onBlur({}, `${name}-${target || "input"}`)
    );
  };
